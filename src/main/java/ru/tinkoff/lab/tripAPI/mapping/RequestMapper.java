...
public interface RequestMapper {
    ...
    @Select("""
            ...
            ORDER BY r.start_date OFFSET #{offset} ROWS FETCH NEXT #{rows} ROWS ONLY;
            """)
    List<Request> selectIncomingStatusRequests(@Param("approverId") UUID approverId, int offset, int rows, String status);
    ...
    @Select("""
            ...
            JOIN destination d on r.destination_id = d.id
            JOIN office o on d.office_id = o.id
            WHERE u.id = '${workerId}' AND r.status = #{status}
            ORDER BY r.start_date OFFSET #{offset} ROWS FETCH NEXT #{rows} ROWS ONLY;
            """)
    List<Request> selectOutgoingStatusRequests(@Param("workerId") UUID workerId, int offset, int rows, String status);
}
